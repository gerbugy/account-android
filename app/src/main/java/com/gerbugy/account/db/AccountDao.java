...
import com.gerbugy.account.util.CipherUtils;
...
public class AccountDao {
    ...
    private final CipherUtils mCipherUtils;

    private AccountDao() {
        mCipherUtils = new CipherUtils(Constants.CIPHER_KEY);
    }
    ...
    private String encrypt(String str) {
        return mCipherUtils.encrypt(str);
    }

    private String decrypt(String str) {
        return mCipherUtils.decrypt(str);
    }

    public long insert(SQLiteItem item) {
        ContentValues values = new ContentValues();
        values.put(Columns.NAME, item.getString(Columns.NAME));
        values.put(Columns.LOGIN_ID, encrypt(item.getString(Columns.LOGIN_ID)));
        values.put(Columns.LOGIN_PASSWORD, encrypt(item.getString(Columns.LOGIN_PASSWORD)));
        values.put(Columns.URL, item.getString(Columns.URL));
        values.put(Columns.DESCRIPTION, encrypt(item.getString(Columns.DESCRIPTION)));
    ...
    public SQLiteItem select(long _id) {
        ...
        if (cursor.moveToNext()) {
            ...
            item.put(Columns.NAME, cursor.getString(index++));
            item.put(Columns.LOGIN_ID, decrypt(cursor.getString(index++)));
            item.put(Columns.LOGIN_PASSWORD, decrypt(cursor.getString(index++)));
            item.put(Columns.URL, cursor.getString(index++));
            item.put(Columns.DESCRIPTION, decrypt(cursor.getString(index++)));
